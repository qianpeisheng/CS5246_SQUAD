...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in example.ner_tag[doc_span.start:doc_span.start + doc_span.length]:
                if i in ner_tags_list:
                    ner_tags.append(per_tags_list.index(i))
                else:
                    ner_tags.append(0)
           ...
           #  pos_tags.extend([pos_tags_list.index(i[1])/len(example.orig_pos_tag) for i in example.orig_pos_tag[doc_span.start:doc_span.start + doc_span.length]])
            ...
            while len(pos_tags) < max_seq_length:
                pos_tags.append(pos_tags_list.index('PH'))
            while len(ner_tags) < max_seq_length:
                ner_tags.append(ner_tags_list.index('PH'))
...
def main():
    ...
    if args.do_train:
        ...
        all_ner_tags = torch.tensor([f.ner_tags for f in train_features], dtype=torch.long)

        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                   all_start_positions, all_end_positions, all_pos_tags, all_ner_tags)
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, start_positions, end_positions, pos_tags, ner_tags = batch
                #print(pos_tags.size())
                loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=[input_mask,pos_tags, ner_tags], start_positions=start_positions, end_positions=end_positions)
                if n_gpu > 1:
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_ner_tags = torch.tensor([f.pos_tags for f in eval_features], dtype=torch.long)

        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_example_index, all_pos_tags, all_ner_tags)
